"""Compare dense attention with a CPU/Gloo distributed attention prototype."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare dense attention with a CPU/Gloo distributed attention prototype."
    )
    ...
    if not _has_torchrun_env():
        print("Distributed causal attention demo")
        print("Run with:")
        print("  python examples/compare_distributed_attention.py --spawn 2")
        print("This demo uses CPU/Gloo module-level attention prototypes.")
...
def _run_demo() -> None:
    ...
    try:
        ...
        result = _compare_attention(hidden_size, num_heads, block_size)
        ...
        if rank == 0:
            ...
            print(f"head_dim: {result['head_dim']}")
            print()
            print("Forward")
            ...
            print()
            print("Note:")
            print("  This is a CPU/Gloo module-level distributed attention prototype.")
            print("  GPT/model real distributed tensor parallelism is not wired yet.")
...
def _compare_attention(hidden_size: int, num_heads: int, block_size: int) -> dict[str, object]:
    ...
    dense.eval()
    distributed = DistributedCausalSelfAttention(
